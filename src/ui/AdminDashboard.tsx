...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-4" role="dialog" aria-label="Panel de administración">
      <div className="bg-[#0a111c] border border-[#1e2d42] rounded-xl w-full max-w-4xl max-h-[90vh] overflow-hidden shadow-2xl flex flex-col">
        ...
        <div className="p-5 overflow-y-auto flex flex-col gap-6">
          ...
          {/* Organizations Table */}
          <div>
            <h3 className="text-xs font-bold text-[#9aaec9] uppercase mb-2">Organizaciones</h3>
            <div className="overflow-x-auto">
              <table className="w-full text-xs" role="table">
                <thead>
                  <tr className="text-[#5a7390] border-b border-[#1e2d42]">
                ...
                <tbody>
                  {loading ? (
                    <tr><td colSpan={5} className="text-center py-4 text-[#5a7390]">Cargando...</td></tr>
                  ) : orgs.length === 0 ? (
                    <tr><td colSpan={5} className="text-center py-4 text-[#5a7390]">Sin datos</td></tr>
                  ) : orgs.map((org: any) => (
                    <tr key={org.id} className="border-b border-[#1e2d42]/40 hover:bg-[#0d1520]/50">
                      <td className="py-2 px-2 text-white font-mono">{org.id}</td>
                      <td className="py-2 px-2 text-[#9aaec9]">{org.owner_email}</td>
                      <td className="py-2 px-2"><span className={`px-2 py-0.5 rounded text-[10px] font-bold ${org.plan === 'enterprise' ? 'bg-purple-500/20 text-purple-300' : org.plan === 'professional' ? 'bg-cyan-500/20 text-cyan-300' : 'bg-[#1e2d42] text-[#9aaec9]'}`}>{org.plan}</span></td>
          ...
          {/* Backups */}
          <div>
            <h3 className="text-xs font-bold text-[#9aaec9] uppercase mb-2 flex items-center gap-2"><Download className="w-3.5 h-3.5" /> Backups</h3>
            {backupList.length === 0 ? (
              <p className="text-xs text-[#5a7390]">No hay backups disponibles</p>
            ) : (
              <div className="flex flex-col gap-1">
                {backupList.slice(0, 5).map((b: any) => (
                  <div key={b.name} className="flex items-center justify-between py-1.5 px-2 bg-[#0d1520] rounded border border-[#1e2d42]">
                    <div>
                      <span className="text-[11px] text-white font-mono">{b.name}</span>
                      <span className="text-[10px] text-[#5a7390] ml-2">({(b.size / 1024).toFixed(1)} KB)</span>
                    </div>
