...
export const OnboardingTour: React.FC<OnboardingTourProps> = ({ isOpen, onClose, onComplete }) => {
  ...
  return (
    <div className="fixed inset-0 z-[60] bg-black/90 flex items-center justify-center p-4" role="dialog" aria-label="Tour guiado">
      <div className="bg-[#0a0f18] border border-[#16202f] rounded-xl w-full max-w-md overflow-hidden shadow-2xl">
        <div className="p-8 flex flex-col items-center text-center gap-5">
          <div className={`p-4 rounded-full bg-[#0e1624] border border-[#16202f] ${s.color}`}>
            <Icon className="w-10 h-10" />
          </div>
          <div>
            <h3 className="text-lg font-bold text-white">{s.title}</h3>
            <p className="text-sm text-[#8ea3bd] mt-2 leading-relaxed">{s.desc}</p>
          </div>

          <div className="flex gap-1.5">
            {steps.map((_, i) => (
              <div key={i} className={`w-2 h-2 rounded-full transition ${i === step ? 'bg-cyan-400 w-4' : 'bg-[#16202f]'}`} />
            ))}
          </div>

          <div className="flex gap-3 w-full">
            {step > 0 && (
              <button onClick={() => setStep(s => s - 1)} className="flex-1 py-2 rounded-lg border border-[#16202f] text-[#8ea3bd] text-xs font-bold cursor-pointer hover:bg-[#0e1624] flex items-center justify-center gap-1">
                <ArrowLeft className="w-3.5 h-3.5" /> Anterior
              </button>
            )}
            <button onClick={() => { if (isLast) { onComplete(); onClose(); } else setStep(s => s + 1); }}
              className={`flex-1 py-2 rounded-lg text-xs font-bold cursor-pointer flex items-center justify-center gap-1 ${isLast ? 'bg-emerald-500 text-white' : 'bg-cyan-500 text-[#05070c]'}`}>
              {isLast ? <><Check className="w-3.5 h-3.5" /> Comenzar</> : <><span>Siguiente</span><ArrowRight className="w-3.5 h-3.5" /></>}
            </button>
          </div>

          {!isLast && (
            <button onClick={() => { onComplete(); onClose(); }} className="text-[10px] text-[#5b6f8c] hover:text-white cursor-pointer">
              Saltar tour
            </button>
          )}
        </div>
